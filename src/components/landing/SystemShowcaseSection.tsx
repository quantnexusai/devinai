      ...
      >
        <defs>
          <linearGradient id="line-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#C5705D" stopOpacity="0" />
            <stop offset="50%" stopColor="#C5705D" stopOpacity="0.5" />
            <stop offset="100%" stopColor="#C5705D" stopOpacity="0" />
          </linearGradient>
        </defs>
        <line
          x1="25%"
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <ScrollReveal delay={200}>
            <div className="hidden lg:flex absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-terracotta/10 border border-terracotta/30 rounded-full items-center justify-center z-10">
              <div className="w-20 h-20 bg-terracotta/20 border border-terracotta/40 rounded-full flex items-center justify-center">
                <span className="text-terracotta font-mono text-xs uppercase tracking-wider">Core</span>
              </div>
            </div>
          </ScrollReveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-16 lg:px-16">
            {stackItems.map((item, index) => (
              <ScrollReveal key={item.name} delay={index * 150 + 300} duration={800}>
                <div className="group relative bg-cream border border-sand rounded-xl p-6 lg:p-8 transition-all duration-300 hover:border-taupe hover:shadow-warm">
                  ...
                  {/* Connection dot */}
                  <div className="hidden lg:block absolute w-2 h-2 bg-terracotta rounded-full">
                    <span
                      className={`absolute w-2 h-2 bg-terracotta rounded-full animate-ping ${
        ...
        <ScrollReveal delay={800}>
          <div className="mt-16 lg:mt-20 flex flex-wrap items-center justify-center gap-3">
            {['TypeScript', 'PostgreSQL', 'Tailwind CSS', 'Edge Functions', 'RLS'].map((tech) => (
              <span
                key={tech}
                className="px-4 py-2 bg-cream border border-sand rounded-lg text-gray-600 text-sm font-mono"
              >
                {tech}
              </span>
            ))}
